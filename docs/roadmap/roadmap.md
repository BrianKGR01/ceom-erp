...
**Criterio de salida de la Fase 1:** los 14 puntos anteriores en `[x]`, con sus tests pasando y sus `ANCLA.md` al día. **Estado real (2026-07-15): 13 de 14 en `[x]` con backend + tests completos.** El ítem #1 (Identidad) queda en `[~]` a propósito — su backend está cerrado desde el principio, pero sus pantallas de onboarding dependen de que exista UI, que es la fase que sigue después de esta. No se fuerza ese ítem a `[x]` para no ocultar ese pendiente real.
