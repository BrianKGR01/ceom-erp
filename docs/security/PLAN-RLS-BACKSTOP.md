...
## 13. Etapa 4.a — Identidad real para el Gateway: diagnóstico y diseño (SIN implementar, 2026-07-21)

**Nada de esta sección está aplicado.** Cero cambios de código, cero migraciones, cero escrituras en
la base — es investigación read-only (grep/lectura de archivos, y contra `riertvgnjaujstwyqoom` solo
`SELECT`s de metadatos de schema) seguida de una recomendación de diseño, tal como se pidió. Todo el
SQL/código de §13.6 es **propuesto**, no ejecutado.

### 13.0 Resumen ejecutivo

- **`solicitanteGateway()` mapeado por completo**: 4 call-sites, los 4 en `monitoreo-institucional/actions.ts`,
  los 4 exclusivamente de lectura, los 4 posteriores a `tieneConsentimiento()`. Ver §13.1.
- **Las 3 opciones de §10.4 siguen siendo las correctas** (confirmado independientemente, no solo
  releído) — Opción A (sembrar un usuario real) sigue siendo la recomendable. Pero la investigación de
  esta etapa encontró un matiz que §10.4 no analizó: **cómo de amplio es exactamente el bypass que ese
  usuario real hereda** determina si la Etapa 4.a mejora la seguridad o abre una regresión real. Ver
  §13.2/§13.3 — es la "pregunta incómoda" del pedido, y tiene una respuesta con evidencia, no una
  intuición.
- **Conflicto de documentación real, no cosmético**: `identidad/ANCLA.md:338-341` y
  `monitoreo-institucional/ANCLA.md:94-98` dicen HOY, como "decisión tomada que un agente no debe
  revertir", exactamente lo contrario de la Opción A ("no crear un usuario de sistema real... es
  intencional"). Ver §13.4 — esto necesita una decisión explícita del usuario antes de que cualquiera
  implemente 4.a, no algo que un agente deba resolver por su cuenta silenciosamente.
- **Respuesta honesta a "qué se desbloquea"**: 4.a **no** vuelve a Ventas/Gastos/Nicho-1 migrables sin
  trabajo nuevo — cada uno va a seguir necesitando su propia policy de bypass, igual que ya iban a
  necesitarla para `ceom_admin`. Lo que 4.a elimina es la necesidad de un parche *ad-hoc* y permanente
  por función (patrón `consultarPagosCompraEnPeriodo`) — lo reemplaza por el mismo tipo de trabajo
  mecánico que el checklist 3.e ya prescribe. Ver §13.5.
- **Recomendación final: 4.a se puede separar de 4.b** (no hace falta forzarlas juntas) **si** se
  adopta la variante angosta del diseño (Opción A′, §13.2) en vez de la Opción A tal cual la escribió
  §10.4. Con la Opción A literal (bypass de `ceom_admin` heredado sin cambios), mi lectura honesta es
  que **4.a sola introduce una regresión real de defensa en profundidad** — acotada, pero real — mientras
  4.b no exista. Con A′, no.

### 13.1 Qué es `solicitanteGateway()` hoy — mapa completo

Definición exacta (`src/modules/identidad/actions.ts:264-286`, comentario de diseño en `249-263`):
...
export async function solicitanteGateway(): Promise<UsuarioConRol> {
  ...
  return {
    id: "00000000-0000-0000-0000-000000000000",   // sin fila real en usuarios ni auth.users
    tenantId: CEOM_OPS_TENANT_ID,                   // SÍ es real — el tenant de sistema sembrado en 0005
    rolId: ROL_CEOM_ADMIN_ID,                       // SÍ es real — y el campo `rol` se resuelve con una query real
    // el resto de los campos son valores fabricados en memoria
    ...
  };
}
```

Precisión importante sobre la premisa: **solo `id` es sintético** (UUID de ceros). `tenantId` y `rolId`
apuntan a filas reales (CEOM Ops, rol CEOM Admin). El único round-trip real que hace la función es
`repo.obtenerRolPorId(ROL_CEOM_ADMIN_ID)` — falla ruidosamente (`throw`) si ese rol de sistema no existe.

**Los 4 call-sites (grep exhaustivo sobre `src/`) — todos en `monitoreo-institucional/actions.ts`:**

| Línea | Función exportada | Función de negocio invocada | Módulo | Qué expone |
|---|---|---|---|---|
| `:120` | `tendenciaVentas` | `consultarIngresosPeriodo` | Ventas | `{ingresos, costos}` agregados del período |
| `:138` | `detalleFinanciero` | `flujoCaja`, `estadoResultados`, `costoFijoTotal` | Financiero | flujo de caja / resultados / costos fijos (Financiero cruza internamente `pagos_venta`/`pagos_compra`/`pagos_gasto`, sin tablas propias — mismo patrón que Panel Admin CEOM y Monitoreo Institucional) |
| `:182` | `detalleOperativo` | `listarProducciones`, `consultarMermaPeriodo` | Operativo/Nicho-1 | producciones + costo de merma |
| `:221` | `detalleInventarioOperativo` | `listarInsumos` | Operativo/Nicho-1 | catálogo de insumos |

Las 7 funciones de negocio alcanzadas son **exclusivamente de lectura** (gateadas con
`tienePermiso(solicitante, tenantId, <módulo>, "ver")`, nunca `"crear"/"editar"`). Ningún otro módulo
usa `solicitanteGateway()` — el otro camino "cross-tenant" del plan, Panel Admin CEOM, usa un
`ceom_admin` **real** (fila real en `usuarios`), no este objeto sintético; son dos caminos distintos
que el plan agrupa por compartir el mismo *tipo* de problema, no por compartir código.

**Orden de evaluación exacto en cada función** (verificado en el código, no asumido):
```
1. await tieneConsentimiento(institucionId, tenantId, moduloVeedor)   ← TypeScript, sobre aprobaciones_tenant
2.   si false → return { autorizado: false }                          ← corta ANTES de tocar el módulo de datos
3. const solicitante = await solicitanteGateway()                     ← recién acá se arma la identidad
4. flujoCaja(solicitante, tenantId, periodo)  → llega a la DB          ← consulta real
```
`tieneConsentimiento()` corre **antes** de que exista cualquier identidad/sesión de base de datos para
el Gateway — hoy `solicitanteGateway()` no pasa por `comoCeomAdmin()`/`comoInstitucion()`, es un
objeto en memoria sin `SET LOCAL ROLE`. Esto es la base de la respuesta de §13.3.

**El patrón de excepción que ya existe** — `consultarPagosCompraEnPeriodo()`
(`src/modules/proveedores/actions.ts:486-517`): intenta `comoUsuario(solicitante.id, ...)` primero; si
`current_tenant_id()` no resuelve (`ContextoRlsNoResueltoError`, disparado siempre para el UUID de
ceros porque nunca existe esa fila), cae a `repo.sumarPagosCompraPeriodo(db, ...)` con el cliente
`db` **crudo** (rol `postgres`, sin RLS). El único gate que queda en ese camino es el
`tienePermiso()` de aplicación evaluado *antes* del `try`. Este es el patrón "mecánico" que, según
§11.2 punto 6, cualquier otro módulo alcanzado por el Gateway va a tener que replicar función por
función mientras `solicitanteGateway()` siga siendo sintético.

**Chequeo de aplicación del que depende TODO lo anterior — `tienePermiso()`
(`identidad/actions.ts:77-104`), leído completo para esta etapa:**
```ts
if (solicitante.rol.esRolSistema && solicitante.rolId === ROL_CEOM_ADMIN_ID) {
  return true;   // sin verificar tenantId, sin verificar que solicitante.id exista como fila real
}
```
Esto es **anterior** a toda esta etapa (ya lo usa `ceom_admin` real) y **no** es parte del alcance de
4.a — pero es una pieza de contexto que la sección 13.3 necesita: hoy, `tienePermiso()` confía
ciegamente en la *forma* del objeto (`rolId === ROL_CEOM_ADMIN_ID`), no en que `id` sea una fila real.
Cualquier objeto con esa forma pasa, sin importar de dónde salió. La única razón por la que esto es
seguro hoy es que **nada en el código construye ese objeto excepto `solicitanteGateway()`**, y su uso
está acotado a 4 call-sites de solo lectura, todos posteriores a `tieneConsentimiento()` — es disciplina
de código documentada (comentario de `identidad/actions.ts:260-262`), no un control técnico verificable
por la base de datos.

### 13.2 Las opciones — confirmadas y una variante nueva

§10.4 ya identificó las 3 opciones correctamente (confirmado independientemente en esta etapa, con el
mismo resultado):

| Opción | Consecuencia (confirmada) |
|---|---|
| **A. Sembrar un usuario de sistema real**, `rolId = ROL_CEOM_ADMIN_ID`, encaja mecánicamente en el Caso 2 (`comoCeomAdmin`/`comoUsuario` — **byte a byte la misma función hoy**, confirmado leyendo `contexto.ts:159-181`) sin cambios de mecanismo | Arregla los 2 tests rotos de §9.6. Pero hereda **automáticamente** cualquier bypass de `es_ceom_admin()` que exista hoy o se agregue mañana en cualquier tabla — sin relación con qué necesita realmente el Gateway. Ver §13.3. |
| **B. Tratar como Caso 3 (`comoInstitucion`) extendido** | Confirmado con más detalle (`contexto.ts:189-197`): `comoInstitucion()` no exige `current_tenant_id()`, exige `auth.uid()` — que no consulta ninguna tabla, es un eco del JWT. El UUID de ceros SÍ resolvería sin reventar. Pero la policy de Etapa 4 para instituciones (`tenant_autorizado_para_institucion_actual()`) filtra contra `instituciones.auth_user_id`, y no hay fila de `instituciones` con ese id — no revienta, pero tampoco autoriza nada. No alcanza sin sembrar algo real igual, y sembrar una fila de `instituciones` para "el Gateway" es conceptualmente más forzado (el Gateway no es una institución). Descartada, igual que en §10.4. |
| **C. Mantenerlo fuera de RLS indefinidamente (statu quo)** | Es literalmente el patrón de `consultarPagosCompraEnPeriodo()` — funciona, no escala: cada función de cada módulo que el Gateway alcance necesita su propia excepción, para siempre. |
| **A′ (nueva en esta etapa) — Opción A, pero con bypass de RLS propio y angosto, NO el de `ceom_admin`** | Ver desarrollo abajo. Más trabajo que A, cierra la regresión que A introduce. **Es mi recomendación.** |

**Por qué A′ y no A tal cual — el detalle que §10.4 no llegó a analizar:**

`es_ceom_admin()` (`0031_es_ceom_admin_function.sql`) filtra por **rol**, no por usuario puntual:
```sql
where u.id = auth.uid() and u.eliminado_en is null and u.activo
  and r.id = 'c1027307-...'  -- ROL_CEOM_ADMIN_ID
  and r.es_rol_sistema
```
Si el usuario sembrado para el Gateway tiene `rolId = ROL_CEOM_ADMIN_ID` (necesario para que
`tienePermiso()` lo siga tratando como hoy, §13.1) — **automáticamente** pasa `es_ceom_admin()`
también, en CUALQUIER tabla que tenga o llegue a tener `ceomAdminBypassPolicy()` — hoy las 4 de
Proveedores, mañana Ventas/Gastos/Financiero-subyacentes/Nicho-1 cuando migren. No hay forma de que la
Opción A, tal cual está escrita en §10.4, le dé al Gateway *menos* que lo que tiene un `ceom_admin`
humano — comparten literalmente la misma función de bypass, por construcción, no por accidente ni por
un bug corregible después.

**A′ separa las dos capas que hoy están fusionadas por necesidad, no por diseño:**
1. **Capa de aplicación (`tienePermiso()`)** — sigue necesitando que `rolId === ROL_CEOM_ADMIN_ID` para
   no tocar código de Identidad fuera de alcance. Pero se angosta: en vez de "true, sin condición",
   agregar `if (solicitante.id === GATEWAY_SISTEMA_USUARIO_ID) return accion === "ver";` — convierte
   "nunca usar para escrituras" de un comentario (`identidad/actions.ts:260-262`) en una regla que
   Postgres... no, que **TypeScript** hace cumplir, verificable con un test.
2. **Capa de RLS** — función y policy **propias**, no reutilizar `es_ceom_admin()`/`ceomAdminBypassPolicy()`:
   `es_gateway_sistema()` (mismo patrón `STABLE SECURITY DEFINER`, filtra por `u.id = 'GATEWAY_SISTEMA_USUARIO_ID'`
   puntual, no por rol) + `gatewaySistemaBypassPolicy(tableName)` con `for: "select"` únicamente (nunca
   `"all"`) — aplicada tabla por tabla, solo donde una función real del Gateway la necesite, en el
   mismo commit que esa tabla migre a `comoUsuario()` (mismo criterio que checklist 3.e, ítem 3).

Costo real de A′ sobre A: una fila de rol nueva (mismo patrón que `0005`, sin riesgo nuevo), una
función SQL más (calco de `es_ceom_admin()`), un helper más en `src/db/rls.ts` (calco de
`ceomAdminBypassPolicy()`), una rama nueva en `tienePermiso()` (5 líneas, testeable), y policies de
bypass que de todas formas había que escribir tabla por tabla (checklist 3.e) — ahora son 2 policies
por tabla en vez de 1 donde el Gateway también la necesite, no N migraciones nuevas.

### 13.3 La pregunta incómoda — respuesta directa

**Con la Opción A tal cual la propuso §10.4: 4.a empeora la seguridad mientras 4.b no exista — de forma
acotada, pero real.** No porque reemplace la decisión de `tieneConsentimiento()` (no la reemplaza, ver
abajo), sino porque **remueve la única resistencia técnica que existe hoy contra un bug que se salte
ese chequeo**, y no pone nada en su lugar.

Mecánica exacta, verificada leyendo el código (no especulada):

- **`tieneConsentimiento()` no se mueve ni se toca con la Opción A.** Sigue siendo 100% código de
  aplicación (`consentimiento/actions.ts:368-382`), sigue evaluándose *antes* de que exista cualquier
  identidad de base de datos para el Gateway (§13.1). Ninguna policy de RLS propuesta en este plan
  (§10.3, §10.8, §12, ni la de esta sección) toca `aprobaciones_tenant` ni conoce `institucionId`/
  `moduloVeedor` — la granularidad por módulo veedor vive donde `docs/architecture/CEOM_Arquitectura.md`
  §8.1 decidió que viviera, en código, deliberadamente (duplicar esa lógica en SQL es exactamente lo
  delicado que separa a la Etapa 4.b, §2.3 Caso 3, "el punto más delicado de todo el plan", cita de §7).
  **4.a no mueve esa decisión — en eso, ni mejora ni empeora nada.**
- **Lo que sí cambia es qué pasa si algo llega a la base de datos SIN haber pasado por
  `tieneConsentimiento()` primero — hoy, por un bug futuro, no uno que exista.** Hoy (`solicitanteGateway()`
  sintético): si algún código nuevo llamara a una función migrada a `comoUsuario()` con este solicitante
  sin el gate de consentimiento, `current_tenant_id()` no resuelve (id de ceros, sin fila) →
  `ContextoRlsNoResueltoError` → **falla ruidosa, visible, sin datos** (o, en un módulo no migrado, al
  menos no hay ningún bypass de RLS que lo salve — el único gate sería el `if` de la propia función).
  Con la Opción A (bypass de `ceom_admin` heredado): esa misma llamada indebida **funcionaría y
  devolvería datos cruzando tenants**, porque `es_ceom_admin()` no sabe nada de consentimiento — solo
  sabe "esta sesión es un `ceom_admin`". El error deja de ser ruidoso y pasa a ser un leak silencioso.
- **Con A′: esa misma llamada indebida sigue sin bypass en ninguna tabla que no tenga explícitamente
  `gatewaySistemaBypassPolicy()` aplicada** (que hoy serían cero, hasta que Ventas/Financiero-subyacentes/
  Nicho-1 migren) **y nunca tiene bypass de escritura en absoluto** (la policy es `for: "select"`, y
  `tienePermiso()` angosto en 13.2 deniega cualquier acción que no sea `"ver"` para este id puntual).
  El blast radius de un bug futuro queda acotado a "puede leer lo que el Gateway ya necesita leer hoy",
  nunca "puede hacer lo que un `ceom_admin` humano puede hacer, incluyendo escribir, en cualquier tabla
  presente o futura".

**Conclusión honesta:** 4.a no es neutral respecto a la seguridad — la dirección depende enteramente de
CÓMO se implemente. Con A (literal) empeora, de una forma específica y acotada (blast radius de un bug
futuro, no una vulnerabilidad activa hoy). Con A′ mejora estrictamente (arregla 2 tests rotos reales,
sin abrir esa regresión) porque el modelo de confianza del Gateway ("un proxy de solo lectura, revocable,
para terceros externos con consentimiento verificado en código") deja de pedirle prestado el modelo de
confianza de Panel Admin CEOM ("operadores humanos de CEOM, exentos de consentimiento por Términos de
Servicio, con acceso total") — que es exactamente la tensión que el pedido original sospechaba y que
esta investigación confirma que es real, no imaginaria.

### 13.4 Conflicto de documentación encontrado — necesita decisión explícita, no un agente resolviéndolo solo

`src/modules/identidad/ANCLA.md:338-341` y `src/modules/monitoreo-institucional/ANCLA.md:94-98`, bajo
el encabezado **"Decisiones tomadas que un agente no debe revertir"**, dicen textualmente:

> "`solicitanteGateway()` es un objeto SINTÉTICO, no una fila de usuario real — no crear un usuario de
> sistema real en la base para 'arreglar' esto [...]" (`identidad/ANCLA.md:338-340`)

> "`solicitanteGateway()` es un objeto sintético, no una fila de usuario real — no intentar 'arreglar'
> esto creando un usuario de sistema real en la base de datos; **es intencional, documentado**, y
> acotado a lecturas mediadas por `tieneConsentimiento()`." (`monitoreo-institucional/ANCLA.md:94-98`)

Esto es **exactamente lo contrario** de la Opción A que este mismo plan (§9.6, §10.4, §10.11 decisión 4)
recomienda. No es una contradicción cosmética — son dos decisiones reales, tomadas en momentos distintos,
con información distinta:

- La decisión de ANCLA.md es de **2026-07-18**, cuando se construyó Monitoreo Institucional — en ese
  momento RLS todavía no filtraba nada de verdad para ningún módulo (Etapas 0-2 recién estaban
  arrancando), así que un objeto 100% en memoria para "prestar" el bypass de `tienePermiso()` (capa de
  aplicación, la única que existía) era simple, suficiente, y correcto para ese contexto.
- El hallazgo de §9.6/§10.4 es de **2026-07-21**, con Proveedores ya migrado a `comoUsuario()` real —
  ahí se descubrió que el objeto sintético **rompe** en cuanto una función que el Gateway alcanza pasa
  a depender de `current_tenant_id()`/RLS real, no solo del chequeo en memoria de `tienePermiso()`. Es
  un problema que no existía cuando se escribió la decisión de ANCLA.md, porque la premisa bajo la que
  esa decisión se tomó ("nunca va a hacer falta que esto pase por RLS de verdad") todavía era cierta en
  ese momento.

**No corresponde a un agente decidir cuál de las dos gana.** Si el usuario acepta 4.a (con A o A′), este
texto de ambos `ANCLA.md` queda **stale** y hay que actualizarlo explícitamente como parte de la
implementación (no basta con cambiar el código y dejar el `ANCLA.md` diciendo lo contrario — AGENTS.md,
regla de "Definición de terminado", ya exige actualizar `ANCLA.md` cuando cambia el contrato de un
módulo). Si el usuario prefiere sostener la decisión original de ANCLA.md, 4.a queda descartada y la
Opción C (statu quo, excepciones permanentes por función) pasa a ser la decisión final, no un default
por omisión.

### 13.5 Qué se desbloquea de verdad — respuesta honesta

**No, Ventas/Gastos/Nicho-1 NO quedan migrables sin trabajo nuevo solo con 4.a.** Confirmado releyendo
§10.7 (ya lo decía) y verificado independientemente contra el código: cada uno de esos módulos va a
necesitar de todas formas su propia policy de bypass (`es_ceom_admin()`/`ceomAdminBypassPolicy()` para
el camino Panel Admin CEOM — eso ya estaba previsto y no cambia con esta etapa) escrita y aplicada en la
MISMA migración que su paso a `comoUsuario()` (checklist 3.e, ítem 3).

**Lo que 4.a sí desbloquea, con precisión:**
- Elimina la necesidad de que cada función alcanzable por el Gateway tenga su propio parche
  `ContextoRlsNoResueltoError` → fallback a `db` crudo (el patrón de `consultarPagosCompraEnPeriodo()`)
  **para siempre**. Con 4.a (cualquiera de las dos variantes), el Gateway pasa a ser un caso más de
  "sesión real con bypass", exactamente la misma forma de trabajo que ceom_admin/Panel Admin CEOM ya
  necesita — no un mecanismo aparte que hay que mantener indefinidamente.
- Con A′ específicamente: el checklist 3.e de cada módulo (Ventas, Gastos, Financiero-subyacentes,
  Nicho-1) gana un ítem mecánico más ("¿esta tabla la toca el Gateway? si sí, agregar también
  `gatewaySistemaBypassPolicy()`") — trabajo repetible, no un rediseño por módulo.
- Arregla, de forma verificable con tests (no solo en teoría), los 2 casos que hoy están rotos
  (§9.6: "Monitoreo Institucional, crash real" — 2 tests en rojo) y habilita corregir los 4 asserts
  débiles de §10.6/§13.8 con datos reales, porque el camino deja de reventar antes de llegar a producir
  un resultado que valga la pena verificar.
- **Lo que 4.a NO desbloquea:** no acerca la Etapa 4.b (policies de portal con vigencia de consentimiento
  re-derivada en SQL) ni un milímetro — son ortogonales por diseño (Caso 2 vs. Caso 3, mecanismos
  distintos, confirmado en §13.1/§13.2). Migrar Ventas/Gastos/Nicho-1 no depende de 4.b en absoluto.

### 13.6 Diseño propuesto — SQL/código (A′, NO aplicado)

**`src/modules/identidad/constants.ts`** (agregar, mismo patrón que las 3 constantes existentes):
```ts
// Fila real sembrada para que solicitanteGateway() deje de ser 100% sintética
// (Etapa 4.a, docs/security/PLAN-RLS-BACKSTOP.md §13) — ver migración de seed.
export const GATEWAY_SISTEMA_USUARIO_ID = "<uuid fijo a generar>";
export const ROL_GATEWAY_SISTEMA_ID = "<uuid fijo a generar>";
```

**Migración de seed** (patrón de `0005_seed_ceom_ops_tenant.sql`, UUIDs fijos literales — NO vía Admin
API/`inviteUserByEmail`, que es el patrón que usan `scripts/seed-*.ts` para usuarios humanos reales:
acá no hay ningún humano a quien invitar por email, y romper esa convención una sola vez, documentado
explícitamente en el propio SQL, es preferible a mandar una invitación real a `sistema@ceom.internal`):
```sql
-- Rol de sistema dedicado (NO reutilizar ROL_CEOM_ADMIN_ID -- ver §13.2/§13.3:
-- la separacion de rol es lo que le da a este bypass un radio de alcance
-- propio, no el de un ceom_admin humano).
insert into public.roles (id, tenant_id, nombre, es_rol_sistema)
values ('<ROL_GATEWAY_SISTEMA_ID>', null, 'Gateway de Consentimiento (sistema)', true);

-- auth.users -- unico caso del repo que inserta ahi por SQL en vez de via
-- Admin API (confirmado: ninguna de las 37 migraciones existentes toca auth.*
-- salvo llamadas a auth.uid() dentro de policies). encrypted_password NULL +
-- banned_until al futuro lejano: la fila existe (auth.uid() la resuelve) pero
-- no hay combinacion de credenciales que produzca una sesion via GoTrue
-- (verificar empiricamente antes de aplicar -- ver 13.7, sub-etapa 1).
insert into auth.users (id, aud, role, email, encrypted_password, banned_until, ...)
values ('<GATEWAY_SISTEMA_USUARIO_ID>', 'authenticated', 'authenticated', null, null, '294276-01-01', ...);

insert into public.usuarios (id, tenant_id, rol_id, nombre_completo, email, activo, eliminado_en)
values ('<GATEWAY_SISTEMA_USUARIO_ID>', '<CEOM_OPS_TENANT_ID>', '<ROL_GATEWAY_SISTEMA_ID>',
        'Gateway de Consentimiento (sistema)', 'sistema@ceom.internal', true, null);
```

**`drizzle/migrations/00XX_es_gateway_sistema_function.sql`** (calco de `0031`):
```sql
...
as $$
  select exists (
    select 1 from public.usuarios u
    where u.id = auth.uid()
      and u.id = '<GATEWAY_SISTEMA_USUARIO_ID>'  -- puntual, NO por rol -- ver §13.2
...
**`src/db/rls.ts`** (agregar junto a `ceomAdminBypassPolicy`):
```ts
export function gatewaySistemaBypassPolicy(tableName: string) {
  return pgPolicy(`${tableName}_gateway_sistema_bypass`, {
    for: "select",                 // NUNCA "all" -- el Gateway solo lee, ver §13.2/13.3
    to: authenticatedRole,
    using: sql`(select es_gateway_sistema())`,   // hoisted desde el dia uno, ver §12
  });
}
```

**`src/db/contexto.ts`** (agregar junto a `comoCeomAdmin`, mismo cuerpo mecánico — Caso 2 le queda
exacto, confirmado en §13.1/§13.2):
```ts
...
```
Necesita agregar `"comoGatewaySistema"` a `FUNCIONES_DE_CONTEXTO` en `contexto.test.ts` (Test B, línea
68-73) a mano — confirmado que el guard no lo detecta solo — y el nuevo call-site de
`monitoreo-institucional/actions.ts` a `ALLOWLIST_IMPORTA_CONTEXTO`.

**`identidad/actions.ts`** — `solicitanteGateway()` deja de fabricar el objeto, devuelve la fila real:
```ts
export async function solicitanteGateway(): Promise<UsuarioConRol> {
  const usuario = await repo.obtenerUsuarioConRolPorId(GATEWAY_SISTEMA_USUARIO_ID);
  if (!usuario) {
    throw new Error("Usuario de sistema del Gateway no encontrado — seed de sistema faltante.");
  }
  return usuario;
}
```

**`identidad/actions.ts` — `tienePermiso()`** (angostar, ver §13.2):
```ts
if (solicitante.id === GATEWAY_SISTEMA_USUARIO_ID) {
  return accion === "ver";   // nunca crear/editar/anular_ajustar para este id puntual
}
if (solicitante.rol.esRolSistema && solicitante.rolId === ROL_CEOM_ADMIN_ID) {
  return true;
}
...
**`monitoreo-institucional/actions.ts`** — envolver cada una de las 4 llamadas en
`comoGatewaySistema()` en vez de pasar el `solicitante` sintético directo (esto depende de que
Ventas/Financiero-subyacentes/Nicho-1 acepten un `Ejecutor`/`tx` en sus funciones, igual prerequisito
que ya aplicó para Patrimonio/Proveedores).

### 13.7 Sub-etapas verificables y reversibles (ninguna aplicada — orden propuesto)

1. **4.a.1 — Sembrar la fila, sin que nada la use todavía.** Migración de seed sola. Verificar en una
   transacción con `rollback` (misma metodología de §8.1/§12.2) que `auth.uid()`/`current_tenant_id()`
   resuelven contra ella, y que un intento de login real (password/magic link/OTP) falla — antes de
   aplicar la migración de verdad. Cero código de app tocado — revert = borrar 1 archivo de migración
   antes de aplicarlo, o `DELETE` por el id fijo después.
2. **4.a.2 — `es_gateway_sistema()` + `comoGatewaySistema()`, sin bypass todavía.** Nueva función SQL +
   nuevo símbolo en `contexto.ts` + actualizar `FUNCIONES_DE_CONTEXTO`. Verificable: los tests de
   allowlist de `contexto.test.ts` siguen en verde con el símbolo nuevo agregado explícitamente.
3. **4.a.3 — `solicitanteGateway()` real + `tienePermiso()` angosto.** Verificable: los 2 tests rotos de
   §9.6 (`monitoreo-institucional.test.ts` caso 2, caso borde 1) pasan a verde. Test nuevo: el id del
   Gateway con `accion !== "ver"` devuelve `false`.
4. **4.a.4 — `gatewaySistemaBypassPolicy()` tabla por tabla**, repartida en el checklist 3.e de cada
   módulo (Ventas, Financiero-subyacentes, Nicho-1) a medida que migre — no es un paso único de esta
   etapa, es un ítem nuevo del checklist recurrente.
5. **4.a.5 — Los 4 asserts débiles + tests negativos nuevos** (§13.8) — recomendado antes de cerrar,
   mismo criterio que §10.9.

### 13.8 Tests a escribir ANTES de tocar código

1. Los 2 tests ya rojos de `monitoreo-institucional.test.ts` (§9.6) — hoy fallan, se vuelven el
   criterio de aceptación de que 4.a.3 funcionó.
2. Los 4 asserts débiles (§10.6, confirmados con file:line idéntico en esta etapa: `:156`, `:151-156`,
   `:198-206`, `:158-161`) — requieren sembrar datos reales de negocio en el `beforeAll` (hoy solo
   siembra tenant+owner+plan+institución+cartera, nunca ventas/producciones/insumos reales), mismo fix
   que ya se aplicó en `panel-admin-ceom.test.ts` caso 3 (assert de valor exacto, no `typeof`).
3. **Nuevo, específico de A′:** el id del Gateway contra `es_ceom_admin()` — debe dar `false` (prueba
   que la separación de rol realmente separa los bypasses).
4. **Nuevo, específico de A′:** un intento de escritura (`UPDATE`/`INSERT`) con el contexto del Gateway
   sobre una tabla con `gatewaySistemaBypassPolicy()` — debe ser denegado (prueba que `for: "select"`
   se sostiene, no solo se declaró).
5. **Nuevo — prueba end-to-end de la pregunta incómoda, no solo unitaria:** simular el bug hipotético
   (llamar a una función de negocio con la identidad del Gateway SIN haber llamado antes a
   `tieneConsentimiento()`) y confirmar que sigue sin devolver datos cross-tenant — este es el test que
   realmente responde §13.3, no una prueba aislada de `es_gateway_sistema()`.
6. Idempotencia del seed: correrlo dos veces (en un entorno de prueba, nunca contra dev/prod
   directamente) y confirmar que falla ruidosamente (violación de PK), no que duplica silenciosamente.

### 13.9 Blast radius si la identidad sembrada falta o está mal

- **Fila ausente o rol mal sembrado:** `solicitanteGateway()` (rediseñada) tira `Error` explícito — las
  4 funciones de Monitoreo Institucional devuelven error a la Institución, visible, no silencioso.
  Mismo criterio de falla ruidosa que ya rige todo `contexto.ts`.
- **Fila presente pero con `rolId`/bypass mal configurado (ej. alguien "simplifica" reusando
  `ceomAdminBypassPolicy()` en vez de `gatewaySistemaBypassPolicy()`):** este es el escenario que
  realmente importa — silencioso, no truena nada, simplemente el Gateway pasa a tener más alcance del
  que debería. Mitigado por: el test #3/#4 de §13.8 (falla si alguna tabla le da al Gateway más de lo
  declarado), y un comentario "REGLA DURA" explícito en `gatewaySistemaBypassPolicy()` (mismo patrón ya
  usado en `ceomAdminBypassPolicy()` y en `identidad/schema.ts` contra `usuarios`/`roles`) prohibiendo
  reusar `es_ceom_admin()` para este caso.
- **Rollback:** mismo patrón que el resto del plan (§5) — migración `down` de Drizzle borra la policy/
  función/fila por el UUID fijo; revertir `solicitanteGateway()`/`tienePermiso()`/`contexto.ts` a su
  versión anterior (3 archivos, sin tocar DB si solo se revierte código). **Precaución real, ya vivida**
  (§11.1 punto 2): borrar una fila de `usuarios` bajo `CEOM_OPS_TENANT_ID` puede chocar con FKs de
  `logs_acceso_admin_ceom` u otras tablas de auditoría que referencien ese id — revisar antes de un
  rollback en un entorno con datos reales, no asumir que el `DELETE` es limpio.

### 13.10 Decisiones abiertas — necesitan al usuario, no un default de agente

1. **¿A (literal, reusa `es_ceom_admin()`) o A′ (bypass propio y angosto)?** Recomiendo A′ — el costo
   adicional es acotado y cierra la regresión real de §13.3. Si el usuario prefiere A por simplicidad,
   dejarlo explícito que la regresión de §13.3 queda aceptada, no ignorada.
2. **El conflicto de `ANCLA.md` (§13.4)** — ¿se actualiza la decisión documentada, o se sostiene y 4.a
   queda descartada a favor de mantener la Opción C indefinidamente?
3. **¿4.a es trabajo de "cuando se aborde Etapa 4" (como dice §10.11 decisión 4) o se adelanta ahora,
   independiente de 4.b?** Mi lectura: son separables técnicamente (Caso 2 vs. Caso 3, confirmado en
   §13.2) — la pregunta es de secuenciación de trabajo, no de dependencia real.
4. **¿Vale la pena escribir `gatewaySistemaBypassPolicy()` para Proveedores también** (hoy cubierto
   indirectamente por el fallback de `consultarPagosCompraEnPeriodo()`, si Financiero efectivamente
   enruta por ahí — no confirmado con certeza en esta etapa, ver nota abajo), **o dejar ese camino como
   está** (Opción C, acotada a un solo módulo ya migrado) **hasta que se justifique con un caso real**?
5. **Nota de honestidad, no una decisión:** esta etapa no confirmó con certeza si `flujoCaja()`
   (Financiero) enruta sus lecturas de `pagos_compra` a través de `consultarPagosCompraEnPeriodo()`
   (Proveedores, respetando la caja negra de AGENTS.md regla 2) o si accede a esa tabla por otro camino
   — no se investigó al nivel de detalle del propio repository de Financiero. Confirmar esto es un
   prerequisito real antes de escribir el listado exacto de tablas para 4.a.4, no se puede fijar de
   antemano sin ese dato.

### 13.11 Decisiones del usuario (2026-07-21) y resolución del prerequisito (decisión 5)

**Decisiones tomadas, con el encuadre exacto pedido:**

1. **A′.** Confirmada. A literal descartada — la regresión de §13.3 queda formalmente rechazada, no
   aceptada.
2. **`ANCLA.md` actualizado** (`identidad/ANCLA.md`, `monitoreo-institucional/ANCLA.md`) — no se revirtió
   la decisión del 2026-07-18, se preservó el principio ("el Gateway es un lector acotado y revocable,
   no un admin") y se documentó que lo que cambió fue el mecanismo, no el principio, con la fecha exacta
   del cambio de premisa (2026-07-21, cuando Proveedores migró a `comoUsuario()`). Ver el texto completo
   en ambos archivos.
3. **4.a se adelanta ahora**, antes de seguir migrando módulos — no porque vuelva gratis a Ventas/
   Gastos/Nicho-1 (§13.5 ya lo aclaraba), sino para no seguir sumando fallbacks ad-hoc a `db` crudo por
   cada módulo nuevo que el Gateway alcance.
4. **Resuelta por la investigación de la decisión 5 (abajo): SÍ hace falta `gatewaySistemaBypassPolicy()`
   sobre `compras` y `pagos_compra`**, y tiene que aplicarse en el MISMO cambio atómico que 4.a.3 — no
   se puede diferir como "Opción C acotada a un módulo ya migrado". Ver el hallazgo abajo, que revisa
   §13.7.
5. **Investigado — prerequisito resuelto:**

   **`Financiero.flujoCaja()` SÍ enruta por `consultarPagosCompraEnPeriodo()` de Proveedores, confirmado
   por código, no por inferencia:**
   ```
   src/modules/financiero/actions.ts:9  → import { consultarPagosCompraEnPeriodo } from "@/modules/proveedores/actions"
   src/modules/financiero/actions.ts:80 → consultarPagosCompraEnPeriodo(solicitante, tenantId, periodo, opts)
   ```
   Respeta la caja negra (AGENTS.md regla 2) — Financiero nunca toca `pagos_compra`/`compras`
   directamente. Y `repo.sumarPagosCompraPeriodo` (`proveedores/repository.ts:203-224`) confirma las
   tablas exactas: `pagos_compra` INNER JOIN `compras`, filtrado por `compras.tenant_id = tenantId`
   (el filtro explícito de la app, invariante de §12) más `pagos_compra.fecha_pago` en el rango.

   **Hallazgo nuevo, más importante que la respuesta en sí: hay un riesgo de secuenciación que no
   estaba identificado antes de esta investigación.** `repo.sumarPagosCompraPeriodo` devuelve
   `coalesce(sum(...), 0)` — **nunca `NULL`**, por diseño (una compra-tenant sin pagos en el período es
   `0` legítimamente). Esto significa que "RLS filtró todo" y "el tenant legítimamente no tuvo pagos en
   el período" son **indistinguibles** en el resultado.

   Secuencia del problema, si 4.a.3 (solicitante real) se aplicara ANTES de que la policy de bypass
   cubra `compras`/`pagos_compra`:
   - Hoy: `solicitanteGateway()` sintético → `comoUsuario()` no resuelve `current_tenant_id()` (sin fila
     real) → `ContextoRlsNoResueltoError` → cae a `db` crudo (bypass total, sin RLS) → suma correcta.
   - Con 4.a.3 aplicado pero SIN la policy en `compras`/`pagos_compra` todavía: `solicitanteGateway()`
     ahora tiene fila real (tenant CEOM Ops) → `current_tenant_id()` **sí resuelve** (a CEOM Ops, no al
     tenant objetivo) → el `try` YA NO LANZA, no cae al fallback → la query corre bajo RLS real, sin
     ningún bypass que la cubra → el `JOIN`/`WHERE` filtra TODAS las filas (CEOM Ops ≠ tenant objetivo)
     → `coalesce(..., 0)` → **`totalPagado: 0`, silenciosamente, sin error** — un dato incorrecto, no
     un crash. Y el assert débil existente (`typeof flujoCaja === "number"`, §10.6) **no lo detecta**:
     `0` es un `number` válido.

   **Consecuencia práctica, revisa §13.7:** 4.a.3 (identidad real) y la parte de 4.a.4 que cubre
   `compras`+`pagos_compra` **no son sub-etapas independientes que se puedan secuenciar libremente — tienen
   que aterrizar en el mismo cambio atómico**, o el `try` de `consultarPagosCompraEnPeriodo()` tiene que
   seguir cayendo al fallback de `db` crudo explícitamente para el id del Gateway hasta que la policy
   exista (una guarda transitoria, no indefinida). Recomiendo lo primero (atómico) — es más simple y
   evita mantener una tercera variante del fallback por un período transitorio.

   **No se investigó** (fuera del alcance de esta pregunta puntual) si `consultarIngresosPeriodo`
   (Ventas) u `listarProducciones`/`consultarMermaPeriodo`/`listarInsumos` (Operativo/Nicho-1) tienen el
   mismo patrón `coalesce(..., 0)`-que-esconde-RLS-filtrando-todo — dado que ninguno de esos módulos
   está migrado a `comoUsuario()` todavía, hoy no aplica (no hay RLS real en su camino), pero el mismo
   chequeo hay que hacerlo cuando a cada uno le toque migrar (ítem nuevo de checklist 3.e: "¿esta
   función devuelve un valor por defecto tipo `coalesce`/`?? 0` que esconda un filtro de RLS vacío?").

**Requisitos de endurecimiento de `tienePermiso()` (pedidos explícitamente) — cómo quedan atendidos en
el diseño de §13.6, sin dejarlos como intención:**

- **Test dedicado con caso negativo** (nuevo, se agrega a §13.8 como ítem 7): confirmar que
  `GATEWAY_SISTEMA_USUARIO_ID` con `accion !== "ver"` (cualquier valor de `Accion` que exista hoy)
  devuelve `false`; confirmar que NINGÚN otro id real (`ceom_admin` real, Owner, colaborador regular)
  entra por esta rama — iterar sobre usuarios de prueba reales, no solo el caso feliz.
- **Constante única, nunca literal disperso**: `GATEWAY_SISTEMA_USUARIO_ID` vive solo en
  `identidad/constants.ts` (§13.6) — usada en `tienePermiso()`, en `solicitanteGateway()`, y en el
  comentario de la función SQL `es_gateway_sistema()` (que sí necesita el literal bakeado en SQL, mismo
  patrón ya aceptado para `ROL_CEOM_ADMIN_ID` dentro de `es_ceom_admin()` — con el mismo comentario de
  "debe coincidir con `identidad/constants.ts`, no cambiar sin migración que actualice ambos lados").
- **Imposible de ampliar por accidente — deny-by-default para acciones nuevas:** la rama propuesta en
  §13.6 es `if (solicitante.id === GATEWAY_SISTEMA_USUARIO_ID) return accion === "ver";` — es una
  comparación de **igualdad contra un único valor permitido** (allowlist), no una negación de valores
  prohibidos (denylist). Si el enum `Accion` gana un miembro nuevo mañana (ej. `"exportar"`), esa
  comparación sigue siendo `false` para el id del Gateway sin que nadie tenga que acordarse de tocar
  esta función — el default es denegar, por construcción del propio chequeo, no por disciplina de quien
  agregue la acción nueva.
- **Manifiesto de acceso (`src/lib/security/access-manifest.ts`) — revisado, no corresponde agregar una
  entrada nueva.** El manifiesto clasifica funciones exportadas de archivos `"use server"` (Server
  Actions invocables directo por el cliente) — `tienePermiso()` es una función interna de librería, no
  un endpoint. Los 4 endpoints reales que el Gateway alcanza (`app/portal/actions.ts::tendenciaVentasAction`,
  `detalleFinancieroAction`, `detalleOperativoAction`, `detalleInventarioOperativoAction`) YA están en el
  manifiesto (`nivel: "por-recurso"`, líneas 268-275) — 4.a no agrega ni cambia ningún endpoint nuevo,
  así que no hay clave nueva que clasificar. Si en algún momento se expusiera un endpoint nuevo que
  reciba directamente el id del Gateway como parámetro (no debería — es un detalle interno), ahí sí
  correspondería una entrada.

**Estado tras esta resolución:** el prerequisito de la decisión 5 está confirmado y documentado. Sigue
